...
이상한 점은 두 번째 인자의 자료형이 `InputArray`라고 되어 있는 것이다. 사실 `InputArray` 타입은 `Mat`. `vector<T>` 등 다양한 객체를 표현할 수 있다. OpenCV에서 자주 사용되는 자료형. 대체로 `Mat`이라고 생각해도 무방.<sup id="fnref:1"><a href="#fn:1" rel="footnote">1</a></sup> 
